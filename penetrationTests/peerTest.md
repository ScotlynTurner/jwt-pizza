...
| Corrections | _Client-side route guard is missing on the frontend — the SPA renders the create-store form for any visitor. Backend still rejects the actual `POST` since it requires an admin/franchise-admin token, but the form should redirect-to-login for unauthenticated visitors. Fix: add a route guard in `src/app/app.tsx` that checks `user` before rendering `/create-store` and the other franchisee/admin routes._ |
...
| Corrections | _This is the intended design: the admin-only `POST /api/franchise` endpoint accepts an `admins` array so an operator can designate any existing user as the franchise admin. Scotlyn's request was made with an admin Bearer token (verified by the successful response). A non-admin caller is blocked at the server (403). No code change required — noting here for the record._ |
...
| Corrections | _Defense held. The login endpoint parameterizes both the email and password lookups in `DB.getUser` (`SELECT * FROM user WHERE email=?`), so the injected `SELECT * from user` payload in the password field is treated as a literal password, `bcrypt.compare` fails, and the server returns `404 unknown user`._ |
...
| Corrections | _The slowdown Scotlyn observed is the intended effect of the hardened `express-rate-limit` on `/api/auth` (10 requests per 5-minute window, keyed on real client IP via `trust proxy`). After the first ~10 attempts the limiter starts returning 429s, which is why subsequent requests take longer and show uneven timing. Defense is working as designed._ |
...
| Corrections | _Partial credit: Scotlyn's curl did successfully create an order record, but the persisted line-item price in the response is `0.0038` (the real Veggie menu price), not the attacker-supplied `-500`. `POST /api/order` now re-fetches the menu server-side and drops the client's `price` and `description` fields before inserting. The order was created, but the price-manipulation attack itself was blocked. Worth discussing in the combined summary — if Scotlyn reads the response carefully, she'll see the `"price":0.0038` override in her own screenshot._ |
...
Ammon focused almost entirely on the backend. His self-attack hardening was server-side work across the board: parameterizing the `updateUser` SQL, adding the missing admin check on `DELETE /api/franchise/:id`, re-fetching menu prices so the client can't set them, stripping stack traces from 5xx responses, adding `helmet`, rate-limiting `/api/auth`, setting a JWT expiry. When he attacked Scotlyn's site, that same mindset carried over, and the bugs he found were all in the service layer: the same SQL injection in `updateUser`, a missing admin check on `GET /api/user` that let any diner pull 76 users and their password hashes, and the same missing-role check on franchise delete. Twelve attacks on her site, ten successful, not one of them on the frontend.

Scotlyn went the other direction. She approached this like a user, not like a developer reading API specs. She brute-forced the login page with Burp Intruder using a common-password wordlist (something Ammon didn't try; his rate-limit test used random strings), she probed pages by typing URLs directly into the browser, and the one real finding she landed on Ammon's site was exactly what that mindset is good at catching: `/franchise-dashboard/create-store` renders for any unauthenticated visitor because the SPA router doesn't gate the route. The backend still rejects the real `POST`, so the data is safe, but the frontend shows the admin form to anyone who types the URL. Ammon had logged this during his own recon and never counted it as an attack, because he was thinking in terms of API endpoints, not user journeys.
...
The `updateUser` SQL injection was the single highest-impact bug on either site. Ammon patched it during his self-attack hardening; Scotlyn still has it. It's a good reminder that string interpolation into SQL is never fine for now. A single quote in a user-controlled field is the whole attack.

Defense in depth paid off in both directions. Two of Ammon's attacks on Scotlyn (JWT forgery with the class-default secret, mass-assignment on register) were blocked by server-side defenses she had in place. Two of Scotlyn's attacks on Ammon (SQL injection via the login password field, brute force) were blocked by two independent defenses each: parameterized queries and `express-rate-limit`. Neither of us wanted to rely on only one layer, and in both directions the second layer is what saved us.

One attack documented as successful turned out not to be, and that's worth flagging. Scotlyn's negative-price order on Ammon's site created an order record, which looks like a win, but the response body shows `"price":0.0038`, not the `-500` she sent. Ammon's server now re-fetches the menu and drops the client `price` field before persisting. The request landed; the attack itself didn't. Easy to miss if you're just checking for a 200 and not reading the body.
