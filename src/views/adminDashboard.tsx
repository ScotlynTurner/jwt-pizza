...
export default function AdminDashboard(props: Props) {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        const res = await pizzaService.getFranchises(franchisePage, 3, '*');
        if (!mounted) return;
        setFranchiseList({
          users: undefined, // keep type shape — replace with actual fields for FranchiseList
          franchises: Array.isArray((res as any).franchises) ? (res as any).franchises : [],
          more: !!(res as any).more,
        } as any);
      } catch (err) {
        console.error('Failed to load franchises', err);
        if (mounted) setFranchiseList({ franchises: [], more: false });
      }
    })();
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        const res = await pizzaService.getUsers(userPage, 5, '*');

        // Defensive normalization: ensure an object with users array + more flag
        const normalized = {
          users: Array.isArray((res as any).users) ? (res as any).users : [],
          more: !!(res as any).more,
        };
        ...
        if (mounted) setUserList({ users: [], more: false });
      }
    })();
    return () => { mounted = false; };
  }, [props.user, userPage]); // <-- fixed to userPage
  ...
  async function filterUsers() {
    try {
      const res = await pizzaService.getUsers(userPage, 10, `*${filterUserRef.current?.value}*`);
      setUserList({
        users: Array.isArray((res as any).users) ? (res as any).users : [],
        more: !!(res as any).more,
      });
    } catch (err) {
      console.error('filterUsers failed', err);
